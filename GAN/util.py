...
folder = "./GAN/models/"
gen_model = tf.keras.models.load_model(folder+'bird_model.h5')
...
def generate_random_word_vectors_from_dataset(n_samples,captions, create_new_captions = False):
...
def generate_latent_points(latent_dim, n_samples,captions, interpolate = False):
    x_input  = tf.random.normal([n_samples, latent_dim])
    text_captions = generate_random_word_vectors_from_dataset(n_samples,captions=captions, create_new_captions=False)
    if interpolate:
